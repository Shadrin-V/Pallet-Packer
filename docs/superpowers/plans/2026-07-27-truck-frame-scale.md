...
> ⚠️ **ВЫПОЛНЕН 2026-07-27** (PR #37, `d1113e3` в `main`, задача `LKWkalk-6n4`). Истина о том, КАК
> оно устроено, — спека `docs/superpowers/specs/2026-07-27-truck-frame-scale-design.md` и код; этот
> план — исторический документ. Реальность разошлась с ним в двух местах:
> 1. **Тестов с явной шириной оказалось три, а не два.** Задача 2, шаг 6 перечисляет тесты переноса
>    ПЛИТОК; на прогоне упал ещё и тест переноса **ЗАГОНОВ** (`warehouseLayout.test.ts`, «загоны
>    переносятся на новую строку»), сужавший кузов тем же способом. Починен так же — явной `width`.
> 2. **Базовая линия «617 зелёных» — не гарантия.** Замер на чистом `main` в отдельном ворктри: он
>    сам роняет 2 теста `SetupScreen`, а два прогона на ветке роняли РАЗНЫЕ пары. Нестабильность
>    пре-существующая и нагрузочная (`LKWkalk-w9j`). Гейт: падение засчитывать, только если тест
>    валится в ОДИНОЧНОМ прогоне своего файла. Полный прогон на ветке дал **622/622**.
>
