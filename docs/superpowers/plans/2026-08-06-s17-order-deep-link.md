...
- **Импорт ДОБАВЛЯЕТ заказ**, не заменяет черновик. **Уточнено на финальном ревью (2026-08-06,
  решение владельца):** исключение — ровно одна НЕТРОНУТАЯ стартовая заготовка (`isPristineDraft`),
  она ЗАМЕЩАЕТСЯ. Иначе на чистом браузере пустая строка заготовки блокировала бы «Рассчитать» рядом
  с готовым импортированным заказом. Полная формулировка — в спеке, решение владельца 1.
