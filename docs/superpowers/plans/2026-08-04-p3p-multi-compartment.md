...
- **Доказанный RED обязателен** (решение владельца, 2026-08-04, после задач 6 и 7). Каждый новый или изменённый тест проверяется против кода **до** этой задачи: временно подменить изменяемый модуль его дореализационной версией (`git show <BASE>:<путь>`), прогнать, зафиксировать падение с конкретным выводом, вернуть реализацию и **сверить диффом, что откат чистый**. Причина: в задачах 6 и 7 тесты, скопированные из плана, проходили и на старом коде — в первом случае из-за совпадения чисел, во втором потому, что позицию уже ловила правка предыдущей задачи. Обе дыры нашлись только на ревью. Если тест отличить старое от нового принципиально не может, его не выдумывают, а честно называют регрессионной гарантией **в заголовке** и объясняют почему.
...
- **`npm run typecheck` из корня — обязательная часть проверки каждой задачи, наравне с тестами.** `npm test` гоняет vitest, который **не проверяет типы**: ошибка `TS2304` в тестовом файле задачи 4 пережила и реализацию, и ревью, и была найдена только четыре задачи спустя. Зелёный `npm test` ничего не говорит о типах.
...
> **Обязательно перед задачами 10–13:** `npm run build --workspace @shadrin-v/engine`. `apps/web` резолвит движок через `packages/engine/dist` (`main`/`types` в его `package.json`), а `dist` в `.gitignore` и после правок ядра остаётся протухшим. Без пересборки импорт `Compartment`, `compartmentsOf` и прочего из `@shadrin-v/engine` в `apps/web` не разрешится, а `ENGINE_CONTRACT_VERSION` в собранном пакете останется старым. Проверка свежести: `grep -c compartmentsOf packages/engine/dist/index.js` должно быть больше нуля.
