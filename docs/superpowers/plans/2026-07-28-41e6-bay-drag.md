...
| `apps/web/src/screens/components/WarehouseBay.tsx` | бирка берёт указатель (`onTagDown`) | 2 |
...
- Produces: проп `onTagDown?: (e: React.PointerEvent) => void` у `WarehouseBay`; узел-ручка
  помечен `data-tag-grip`. Без пропа компонент остаётся полностью инертным, как сегодня.

- [ ] **Step 1: Написать падающие тесты**

Добавить в `WarehouseBay.test.tsx`. Фикстура `bay` дополняется полем `rowH: 1530` (если это не
сделано в задаче 1), и рядом с `draw` появляется вариант с обработчиком:
...
const drawGrip = (onTagDown: () => void) =>
  render(
    <svg>
      <WarehouseBay bay={bay} series={2} label={bay.orderId} onTagDown={onTagDown} />
    </svg>,
  ).container;
```
...
```ts
  ...
  it('без обработчика бирка остаётся инертной — двор без переноса загонов', () => {
    const c = draw();
    expect(c.querySelector('[data-tag-grip]')!.getAttribute('pointer-events')).toBeNull();
  });
```
...
Expected: FAIL — узла `[data-tag-grip]` нет, `onTagDown` не вызывается.
...
git add apps/web/src/screens/components/WarehouseBay.tsx \
        apps/web/src/screens/components/WarehouseBay.test.tsx
git commit -m "feat(warehouse): bay tag takes the pointer as a drag grip (36f)"
```
...
```tsx
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (!pt) return;
      setDragBay({ orderId: dragBay.orderId, order: reorderBaysAt(floor, dragBay.orderId, pt) });
    };
    const up = () => {
      onBayOrderChange?.(dragBay.order);
      setDragBay(null);
    };
                ...
                onTagDown={
                  onBayOrderChange
                    ? () =>
                        ...
                        setDragBay({ orderId: bay.orderId, order: floor.bays.map((b) => b.orderId) })
                    : undefined
                }
              />
```
...
- Порядок живёт в `LadeplanScreen` как `bayOrder` — слой поверх умолчания (заявка, `8z2`), а не
  ...
  порядок содержимого плана, а не настройка вида. Клавиатурой загон пока не переставить
  (`LKWkalk-e8x`).
```
...
3. **i18n читается тестами из собранного `dist`** — после правки словаря нужен
   `npm run build -w @shadrin-v/i18n`. В этой задаче словари не трогаются.
