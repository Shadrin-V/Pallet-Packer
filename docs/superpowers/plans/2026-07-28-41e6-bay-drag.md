...
> ⚠️ **ВЫПОЛНЕН 2026-07-28** (PR #41, ветка `feat/36f-bay-drag`, на момент записи ждёт мержа).
> Истина о том, КАК оно устроено, — спека `docs/superpowers/specs/2026-07-28-41e6-bay-drag-design.md`
> и код; этот план — исторический документ, и код-блоки ниже отражают ЗАМЫСЕЛ, а не то, что уехало.
> Против текста ниже уехало четыре вещи:
>
> 1. **Добавились два требования, которых в задаче 4 плана нет** — они родились из ревью задачи 3 и
>    записаны в спеку как решения 6 и 7: жест, не изменивший порядок, не фиксирует ничего (иначе клик
>    по бирке заморозил бы порядок заявки как явный пользовательский), и новый порядок СЛИВАЕТСЯ с
>    прежним, а не заменяет его (иначе заказ, чьи стопки уехали в кузов, терял бы своё место).
>    Ради первого в состояние драга добавился неизменный снимок `from`: сравнение с последним
>    провизорным кадром убило бы вообще все фиксации, оставив тесты живой перекомпоновки зелёными.
> 2. **Финальное ревью нашло настоящий баг за пределами плана.** Выделение стопки (`sel` в
>    `WarehouseFloor`) хранилось индексом в СГРУППИРОВАННОЙ отрисовке — единственный индекс, который
>    не переводили в систему координат родителя. Перестановка загонов уводила кольцо выделения и
>    кнопку ⟳ на чужую стопку; дефект был достижим и через существующий флажок группировки, то есть
>    жил на проде. Перевязан на `realIndex` с регрессионным тестом.
> 3. **Три теста добавлены сверх плана** (ветка «точка выше строки» в `bayIndexAt`, чужой `pointerId`
>    у `pointercancel`, проводка `mergeBayOrder`) — каждый из них до правки оставлял набор зелёным.
> 4. **Задача 5 выполнена частично:** CHANGELOG записан, а `bd close` и проверка прода по маркеру
>    сборки ждут мержа PR.
