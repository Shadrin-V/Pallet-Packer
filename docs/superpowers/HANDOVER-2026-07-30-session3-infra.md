...
> ⚠️ SUPERSEDED (2026-07-30, сессия 4): актуальный вход — `HANDOVER-2026-07-30-session4-visual-batch.md`.
> Раздел «Что взять дальше» ниже ВЫПОЛНЕН в части `6zp`, `5tg` и `103` — не брать их заново.
> Разделы «Состояние» и «Грабли» (Basic Auth, бэкап) остаются верными: прод по-прежнему требует `-u`.
