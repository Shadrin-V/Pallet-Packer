...
> до реализации.** Версия контракта: `0.17.0` (аддитивно; полная история версий — в конце файла).
...
interface Vehicle {
  ...
  length: number;        // мм, > 0 — полный пролёт по длине, ВКЛЮЧАЯ разрывы между отсеками
  width: number;         // мм, > 0
  height: number;        // мм, > 0
  compartments?: Compartment[];  // отсеки по возрастанию x; отсутствует = один отсек [0, length)
  maxPayload?: number;   // кг; опц.; в MVP не проверяется
}
```

### Compartment (0.16.0, [ADR 026](adr/026-multi-compartment-vehicle.md))
```ts
...
Транспорт с несколькими отсеками (автопоезд: тягач + прицеп) моделируется интервалами на **общей**
оси длины транспорта, а не локальными координатами внутри каждого отсека — разбор в
[ADR 026](adr/026-multi-compartment-vehicle.md). Разрыв между отсеками (например, между тягачом и
прицепом) не выделен отдельной сущностью: это промежуток координат оси, не покрытый ни одним
`Compartment`, куда груз не встаёт. Сквозной инвариант, заменяющий проверку габаритов кузова: единица
груза лежит целиком внутри **одного** отсека — не «в габаритах кузова», а «в габаритах отсека».
Проверяется в `findGeometryViolations` и во всех операциях ручной правки. Метрики (`floorArea`,
объём кузова) считаются суммой по отсекам, не по полному `length` — иначе разрыв завысил бы объём
...
(перетаскивание штабелей в UI): единицы не выходят за габариты **своего отсека** (не «кузова» —
при `compartments` единица обязана целиком лежать внутри одного отсека, [ADR 026](adr/026-multi-compartment-vehicle.md)),
не пересекаются (единицы одной `(x,y,cargoTypeId)`-колонки делят колонку законно,
[ADR 014](adr/014-nested-column-geometry.md)), ориентация допустима правилом вращения.
...
Кандидаты: прицел, обе стенки **каждого отсека**, позиции **впритык** к кромкам соседей — внутри
отсека и в пределах `tolerance` от прицела. Выбор: больше «впритык»-осей → ближе к прицелу →
`(x, y)` по возрастанию (детерминизм). Прицел в разрыв между отсеками ни одному отсеку целиком не
принадлежит и в кандидаты не попадает; побочно он примагничивается к ближайшей стенке соседнего
отсека, если попал в `tolerance` ([ADR 026](adr/026-multi-compartment-vehicle.md)).
...
упора — стенки **своего отсека** или чужой напольной стопки; стопки за разрывом между отсеками не
помеха и не цель ([ADR 026](adr/026-multi-compartment-vehicle.md)). Стопки из `refs` друг другу не препятствия;
...
| `ERR_CARGO_EXCEEDS_VEHICLE` | Габарит единицы (в любой разрешённой ориентации) не помещается **ни в один** отсек ([ADR 026](adr/026-multi-compartment-vehicle.md)) |
...
### История версий
- `0.17.0` — экспонированы `VALIDATION_ERROR_CODES` и тип `ValidationErrorCode` — коды уже были
  описаны в §3, изменилась только их доступность из TypeScript; поведение движка не менялось
  (`LKWkalk-p3p.16`).
