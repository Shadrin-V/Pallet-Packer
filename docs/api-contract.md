...
> до реализации.** Версия контракта: `0.6.0` (аддитивно; полная история версий — в конце файла).
...
interface StackPreview {   // единицы в ОДНОМ вертикальном штабеле (ADR 003), до 2D-раскроя пола
  count: number;           // поддонов в штабеле
...
`stepHeight`/режима вложения до полного расчёта; при `H ≤ 0` или `Hк < H` вернёт `count: 0`.
