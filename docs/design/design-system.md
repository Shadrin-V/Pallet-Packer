...
> JSX.** Палитр **две**, обе светлые: `forest` (умолчание, фирменный зелёный Holz Schäfer) и `warm`
...
> по-прежнему оставлен токенами. Правило прежнее: компоненты используют ТОЛЬКО токены. Шрифт —
> **Inter**. Полное обоснование и журнал решений —
...
**Чернильный ряд обеих палитр проходит WCAG 1.4.3** (`--ink` / `--muted` / `--faint` ≥ 4,5:1 к
`--paper`) и ступени не сливаются — это гейт, а не договорённость: `apps/web/src/theme/palette.test.ts`
(2026-08-04). На `--faint` висят единицы, микро-подписи и метки `Vorne/Hinten`, поэтому осветлять его
«ради воздуха» нельзя.

**`--hero-tint`** — хвост градиента шапки, отдельный от `--mint-tint`: тот держит активные состояния
и остаётся зелёным в обеих палитрах, а хвост шапки следует температуре палитры (в `warm` — пшеничный),
иначе бежевое тело живёт под холодной шапкой.
...
- Фон — калька: `--paper` + бледная сетка 1000 мм (`--grid`, opacity .5). Рамка кузова — `--truck`, тоньше кабины (d2d). Инженерная линейка: метры (нумерованные) + полу-/четвертьметровые штрихи; длина — по верхней кромке, высота/ширина — по кормовой; каждая ось с итогом (mr8).
...
```css
@page { size: A4 landscape; margin: 8mm; }   /* печатная область — 281 × 194 мм */
@media print {
  .plan-sheet  { height: 194mm; display: flex; flex-direction: column; }  /* ровно одна страница */
  .plan-sheet > * { flex-shrink: 0; }        /* шапка, сводка, легенда — своя высота */
  .plan-figures { flex: 1 1 auto; min-height: 60mm; }  /* разрезы делят ОСТАТОК страницы */
}
```
- **A4 landscape, ровно одна страница** (`LKWkalk-7hx`). Постоянная часть листа занимает свою
  естественную высоту, разрезы получают остаток: у каждого `svg[data-cutaway]` есть `viewBox` и
  `preserveAspectRatio="xMidYMid meet"`, поэтому чертёж вписывается в выданную коробку целиком, а не
  обрезается. Высота листа — в мм от `@page`, не в `vh`: единицы вьюпорта Chrome в печати считает не
  от страницы.
- Порог: если постоянная часть перерастает страницу (очень длинная легенда — порядка 20 заказов),
  разрезы останавливаются на `min-height: 60mm`, а хвост легенды уезжает на вторую страницу
  (`print:overflow-visible` на листе). Лучше вторая страница, чем срезанная легенда или разрез в
  четыре миллиметра.
- Состав листа: шапка (машина/заказы) + сводка заполнения (паллеты, места, % пола и объёма, не
  размещено) → разрезы сбоку+сверху → легенда заказов. Склад и буфер — `print:hidden`: это
  верстак, а не документ.
...
- Приёмка печати — **счётчик страниц в `Page.printToPDF`** на настоящем Chrome, а не взгляд на
  экран: в jsdom раскладки нет, юнит-тест проверяет только наличие крючков `.plan-sheet`/
  `.plan-figures` в разметке.
