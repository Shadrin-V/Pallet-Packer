...
3. `resolveGroupDrop(load, layout, refs, aim, opts)` обобщает магнит ADR 020. Кандидат —
   ...
   пересекались до сдвига, а сдвиг общий. До поиска дельты каждая участница проходит те же
   позиц-независимые проверки, что у одиночного `resolveDrop` (rotation и fork access против
   ТЕКУЩЕГО `load`; добавлено фиксом `LKWkalk-v1m`, 2026-07-30): без них магнит обещал `ok:true`
   для дельты, которую `moveStacks` отвергнет через `findGeometryViolations`, если `load`
   изменили после расчёта раскладки, — вопреки обещанию ADR 020.
