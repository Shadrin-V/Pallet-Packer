...
export function warehouseFloor(
  ...
  // Поля двора одинаковы со всех сторон. `padLeft`/`padRight` держали асимметрию слева/направо
  // ради доков (LKWkalk-103: левый узкий, правый — с погрузчиком, шире), но растровая сцена с
  // доками была удалена, и просить их по отдельности стало некому — ни одного вызывающего кода,
  // ни одного теста. Опции убраны финальным ревью этой задачи, а не переписаны: сохранять мёртвую
  // асимметрию в комментариях значило приглашать следующего читателя воскресить её и снова
  // сломать инвариант масштаба 1:1 двор↔разрез, который этот файл и так предупреждает беречь.
