...
2. **UI — заменяемый слой.** Первый интерфейс делается в **Lovable** как клиент API движка. В основном репозитории UI-прототип не дорабатывается вглубь — правки интерфейса идут в Lovable, правки логики — в ядре. Ломающее изменение контракта API требует ADR и обновления `docs/api-contract.md` до реализации.
