...
export const ru: Dictionary = {
  ...
  'ladeplan.loadingModeHintLabel': 'Пояснение к режиму загрузки',
  ...
  ERR_INVALID_NESTING:
    'Некорректное вложение: шаг высоты должен быть больше 0 и не больше высоты груза.',
  ...
  ERR_INVALID_COMPARTMENTS:
    'Отсеки кузова заданы некорректно: длины должны быть целыми положительными числами, координаты — целыми числами от 0, по возрастанию и без пересечений (разрыв между ними, например сцепка автопоезда, допустим), id каждого отсека должен быть уникален, а конец последнего отсека — совпадать с концом транспорта.',
